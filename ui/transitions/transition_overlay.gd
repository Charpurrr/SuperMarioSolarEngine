...
##The base class for all transition overlays. To be instanced in a scene file along with an [AnimationPlayer].
##[br]For a list of all available transitions, see [SceneTransition]
##[br]IMPORTANT: Any new overlays MUST have a child [AnimationPlayer] with an animation named [code]"transition"[/code] to work properly!
